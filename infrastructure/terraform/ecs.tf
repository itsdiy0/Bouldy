# ECS Fargate — runs API, Frontend, and Qdrant as serverless containers
# Replaces K8s Deployments + Services
...
# IAM role for ECS tasks (S3 access for document storage)
...
# Service Discovery namespace (so services can find each other by name)
...
resource "aws_ecs_task_definition" "api" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "DATABASE_URL", value = "postgresql://${var.db_username}:${var.db_password}@${aws_db_instance.postgres.endpoint}/bouldy" },
      ...
      { name = "MINIO_BUCKET", value = aws_s3_bucket.documents.id },
      { name = "MINIO_SECURE", value = "true" },
      { name = "REDIS_URL", value = "redis://${aws_elasticache_cluster.redis.cache_nodes[0].address}:6379" },
      { name = "OPENAI_EMBEDDING_KEY", value = var.openai_embedding_key },
